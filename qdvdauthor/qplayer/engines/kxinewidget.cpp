...
#include <QTimer>		//xxx
//#include <qevent.h>		//ooo
#include <QEvent>		//xxx
//#include <qthread.h>		//ooo
#include <QThread>		//xxx
//#include <qdir.h>		//ooo
#include <QDir>			//xxx
//#include <qcursor.h>		//ooo
#include <QCursor>		//xxx
//#include <qimage.h>		//ooo
#include <QImage>		//xxx
//#include <qdatetime.h>	//ooo
#include <QDateTime>		//xxx
//#include <qtextcodec.h>	//ooo
#include <QTextCodec>		//xxx
//Added by qt3to4:
#include <QTimerEvent>
#include <QWheelEvent>
#include <QMouseEvent>
...
KXineWidget::KXineWidget(QWidget* parent, const char* name,
		...
		//: QWidget(parent,name), m_startXineManual(startManual), m_xineReady(false),		//ooo
		: QWidget(parent), m_startXineManual(startManual), m_xineReady(false),			//xxx
		...
		m_currentZoom(100), m_currentZoomX(100), m_currentZoomY(100), m_currentAudio(0), m_currentSub(0), m_savedPos(0), m_autoresizeEnabled(false)
{
	...
	//setPaletteBackgroundColor(QColor(0,0,0)); //black	//ooo
	...
	if (pathToConfigFile.isNull())
	{
		...
		//m_configFilePath = QDir::homeDirPath();	//ooo
		m_configFilePath = QDir::homePath();		//xxx
...
void KXineWidget::saveXineConfig()
{
	...
	if (!m_cachedCDPath.isNull())
	{
		...
		//config.str_value = (char*)m_cachedCDPath.latin1();		//ooo
		config.str_value = m_cachedCDPath.toLatin1().data();		//xxx
	...
	if (!m_cachedVCDPath.isNull())
	{
		...
		//config.str_value = (char*)m_cachedVCDPath.latin1();		//ooo
		config.str_value = m_cachedVCDPath.toLatin1().data();		//xxx
		xine_config_update_entry (m_xineEngine, &config);
	}
	...
	if (!m_cachedDVDPath.isNull())
	{
		...
		//config.str_value = (char*)m_cachedDVDPath.latin1();		//ooo
		config.str_value = m_cachedDVDPath.toLatin1().data();		//xxx
		xine_config_update_entry (m_xineEngine, &config);
	}

	debugOut(QString("Save xine config to: %1").arg(m_configFilePath));
	//xine_config_save(m_xineEngine, m_configFilePath.ascii());		//ooo
	xine_config_save(m_xineEngine, m_configFilePath.toLatin1().data());	//xxx
}
...
void KXineWidget::xineEventListener(void *p, const xine_event_t* xineEvent)
{
	...
	switch (xineEvent->type)
	{
		...
		case XINE_EVENT_UI_CHANNELS_CHANGED:  /* new channel informations */
		{
			...
			channels = xine_get_stream_info(vw->m_xineStream, XINE_STREAM_INFO_MAX_AUDIO_CHANNEL)+1;
			...
			channels = xine_get_stream_info(vw->m_xineStream, XINE_STREAM_INFO_MAX_SPU_CHANNEL)+1;
	...
	switch ( tevent->timerId() )
	{
		...
		case TIMER_EVENT_CHANGE_CURSOR:
		{
			if (m_DVDButtonEntered)
				setCursor(QCursor(Qt::PointingHandCursor));	//ooo
				//QCursor().setShape(Qt::PointingHandCursor);	//xxx
			else
				setCursor(QCursor(Qt::ArrowCursor));		//ooo
				//QCursor().setShape(Qt::ArrowCursor);		//xxx
			break;
		}
...
bool KXineWidget::initXine()
{
	...
	//m_audioInfo = strdup(i18n("Audiodriver to use (default: auto)").local8Bit());		//ooo
	m_audioInfo = strdup(i18n("Audiodriver to use (default: auto)").toLocal8Bit().data());		//xxx
	...
	//m_videoInfo = strdup(i18n("Videodriver to use (default: auto)").local8Bit());		//ooo
	m_videoInfo = strdup(i18n("Videodriver to use (default: auto)").toLocal8Bit().data());		//xxx
	...
	//m_mixerInfo = strdup(i18n("Use software audio mixer").local8Bit());		//ooo
	m_mixerInfo = strdup(i18n("Use software audio mixer").toLocal8Bit().data());		//xxx
	...
	//m_osdShowInfo = strdup(i18n("Show OSD Messages").local8Bit());	//ooo
	m_osdShowInfo = strdup(i18n("Show OSD Messages").toLocal8Bit().data());	//xxx
	...
	//m_osdSizeInfo = strdup(i18n("Size of OSD text").local8Bit());		//ooo
	m_osdSizeInfo = strdup(i18n("Size of OSD text").toLocal8Bit().data());		//xxx
	...
	//m_osdFontInfo = strdup(i18n("Font for OSD Messages").local8Bit());	//ooo
	m_osdFontInfo = strdup(i18n("Font for OSD Messages").toLocal8Bit().data());	//xxx
	...
	m_videoDriver = xine_open_video_driver(m_xineEngine,
	                                       //m_videoDriverName.ascii(),  XINE_VISUAL_TYPE_X11,	//ooo
					       m_videoDriverName.toLatin1(),  XINE_VISUAL_TYPE_X11,	//xxx
	                                       (void *) &(m_x11Visual));
	...
	//m_audioDriver = xine_open_audio_driver(m_xineEngine, m_audioDriverName.ascii(), NULL);	//ooo
	m_audioDriver = xine_open_audio_driver(m_xineEngine, m_audioDriverName.toLatin1(), NULL);	//xxx
...
//bool KXineWidget::playDvb()	//ooo
int KXineWidget::playDvb()	//xxx
{
#ifndef USE_QT_ONLY
	unwireAudioFilters();

	//Q3PtrList<PostFilter> activeList;	//ooo
	QList<PostFilter> activeList;		//xxx
...
//void getOSDLine( xine_osd_t *osd, int w, Q3CString &dest, Q3CString &source )		//ooo
//void getOSDLine( xine_osd_t *osd, int w, QByteArray &dest, QByteArray &source )	//xxx
void getOSDLine( xine_osd_t *osd, int w, QString &dest, QString &source )		//xxx
{
	int prevPos, pos, tw, th;
	bool wrap=false;

	//pos = source.find(" ");	//ooo
	pos = source.indexOf(" ");	//xxx
	if ( pos==-1 ) {
		dest = source;
		source = "";
		return;
	...
	while ( !wrap ) {
		//xine_osd_get_text_size( osd, dest, &tw, &th );			//ooo
		xine_osd_get_text_size( osd, dest.toLatin1().data(), &tw, &th );	//xxx
		if ( tw>w ) {
			wrap = true;
			break;
		}
		if ( pos==-1 )
			break;
		prevPos = pos;
		//pos = source.find(" ",pos+1);		//ooo
		pos = source.indexOf(" ",pos+1);	//xxx
		dest = source.left( pos );
	}
	if ( wrap ) {
...
void KXineWidget::dvbShowOSD()
{
	...
	if ( dvbOSD ) {
		...
		int tw, th, len;
		//xine_osd_get_text_size(dvbOSD, t.utf8(), &tw, &th);	//ooo
		xine_osd_get_text_size(dvbOSD, t.toUtf8(), &tw, &th);	//xxx
		len = tw;
		int offset = 5;
		//xine_osd_draw_text( dvbOSD, w-(2*border)-tw-offset, 5, t.utf8(), DVB_TEXT_BLUE );	//ooo
		xine_osd_draw_text( dvbOSD, w-(2*border)-tw-offset, 5, t.toUtf8(), DVB_TEXT_BLUE );	//xxx
		int i;
		...
		while ( i<(int)t.length() ) {
			//xine_osd_get_text_size( dvbOSD, ct, &tw, &th );			//ooo
			xine_osd_get_text_size( dvbOSD, ct.toLatin1().data(), &tw, &th );	//xxx
			if ( tw<=(w-(2*border)-offset-5-len) ) break;
			ct = ct.remove( ct.length()-1, ct.length() );
			i++;
		}
		//xine_osd_draw_text( dvbOSD, offset, 5, ct, DVB_TEXT_BLUE );			//ooo
		xine_osd_draw_text( dvbOSD, offset, 5, ct.toLatin1().data(), DVB_TEXT_BLUE );	//xxx
		...
		if ( dvbCurrentNext[0]=="E" ) {
			...
			if ( !dvbCurrentNext[1].isEmpty() ) {
				...
				t = s;
				//xine_osd_draw_text( dvbOSD, 10, y, c.utf8(), DVB_TEXT_GREEN );	//ooo
				xine_osd_draw_text( dvbOSD, 10, y, c.toUtf8(), DVB_TEXT_GREEN );	//xxx
				//xine_osd_get_text_size( dvbOSD, c.utf8(), &offset, &th );		//ooo
				xine_osd_get_text_size( dvbOSD, c.toUtf8(), &offset, &th );		//xxx
				i=0;
				//cs = s.utf8();	//ooo
				cs = s.toUtf8();	//xxx
				while ( i<(int)t.length() ) {
				...
				//xine_osd_draw_text( dvbOSD, 10+offset, y, ct, DVB_TEXT_WHITE );			//ooo
				xine_osd_draw_text( dvbOSD, 10+offset, y, ct.toLatin1().data(), DVB_TEXT_WHITE );	//xxx
				y+= 40;
			}
			if ( !dvbCurrentNext[2].isEmpty() ) {
				//cs = dvbCurrentNext[2].utf8();	//ooo
				cs = dvbCurrentNext[2].toUtf8();	//xxx
				while ( y<(h-(2*border)-23) ) {
					getOSDLine( dvbOSD, (w-(2*border)-20), ct, cs );
					//xine_osd_draw_text( dvbOSD, 10, y, ct, DVB_TEXT_BLUE );			//ooo
					xine_osd_draw_text( dvbOSD, 10, y, ct.toLatin1().data(), DVB_TEXT_BLUE );	//xxx
					y+= 28;
					if ( !cs.length() )
						break;
				}
			...
			if ( !dvbCurrentNext[3].isEmpty() ) {
				//cs = dvbCurrentNext[3].utf8();	//ooo
				cs = dvbCurrentNext[3].toUtf8();	//xxx
				while ( y<(h-(2*border)-23) ) {
					getOSDLine( dvbOSD, (w-(2*border)-20), ct, cs );
					//xine_osd_draw_text( dvbOSD, 10, y, ct, DVB_TEXT_WHITE );			//ooo
					xine_osd_draw_text( dvbOSD, 10, y, ct.toLatin1().data(), DVB_TEXT_WHITE );	//xxx
					y+= 28;
					if ( !cs.length() )
						break;
				}
		...
		xine_osd_show( dvbOSD, 0 );
		//dvbOSDHideTimer.start( 5000, true );		//ooo
		dvbOSDHideTimer.start( 5000 );			//xxx
	}
}
...
//bool KXineWidget::openDvb()	//ooo
int KXineWidget::openDvb()	//xxx
{
	if ( dvbOSD ) {
		dvbOSDHideTimer.stop();
		xine_osd_hide( dvbOSD, 0 );
	...
	//else fprintf(stderr,"xine pipe opened %s\n", m_trackURL.ascii());		//ooo
	else fprintf(stderr,"xine pipe opened %s\n", m_trackURL.toLatin1().data());	//xxx
	m_trackURL = "DVB";
...
void KXineWidget::slotPlay()
{
	...
	//setCursor(QCursor(Qt::WaitCursor));	//ooo
	QCursor().setShape(Qt::WaitCursor);	//xxx

	m_trackURL = m_queue.first();
	//m_queue.remove(m_queue.find(m_trackURL));	//ooo
	m_queue.removeAt(m_queue.indexOf(m_trackURL));	//xxx
	...
	//for (int i = 1; i <= m_trackURL.contains('#'); i++) {		//ooo
	for (int i = 1; i <= m_trackURL.indexOf('#'); i++) {		//xxx
	...
	//debugOut(QString("Playing: %1").arg(m_trackURL.local8Bit()));			//ooo
	debugOut ( QString ( "Playing: %1" ).arg( m_trackURL ) );			//xxx
	...
	{
		//if (m_visualPluginName && (!m_xinePost))			//ooo
		if ( ! m_visualPluginName.isEmpty ( ) && (!m_xinePost))		//xxx
		{
			debugOut(QString("Init visual plugin: %1").arg(m_visualPluginName));
			//m_xinePost = xine_post_init(m_xineEngine, m_visualPluginName, 0,			//ooo
			m_xinePost = xine_post_init(m_xineEngine, m_visualPluginName.toLatin1().data(), 0,	//xxx
			                            &m_audioDriver,
			                            &m_videoDriver);
	...
	//QTextCodec *CodecUtf8;				//ooo
	//CodecUtf8 = QTextCodec::codecForName("UTF-8");	//ooo
	...
	if (currentUtf8Locale)
		...
		//m_trackTitle = QString::fromLocal8Bit(infotag.ascii());		//ooo 
		m_trackTitle = QString::fromLocal8Bit(infotag.toLatin1().data());	//xxx

	//if (CodecUtf8->heuristicContentMatch(infotag.ascii(), infotag.length()) >= 0)			//ooo
	//if (CodecUtf8->heuristicContentMatch(infotag.toLatin1().data(), infotag.length()) >= 0)		//xxx
		//m_trackTitle = QString::fromUtf8(infotag.ascii());					//ooo
		//m_trackTitle = QString::fromUtf8(infotag.toLatin1().data());				//xxx
	...
	if ((!m_trackTitle.isNull()) && (!m_trackTitle.isEmpty())) /* no meta? */
	{
		...
		{
			//m_trackArtist = QString::fromLocal8Bit(trackArtist.ascii());			//ooo
			m_trackArtist = QString::fromLocal8Bit(trackArtist.toLatin1().data());		//xxx
			//m_trackAlbum = QString::fromLocal8Bit(trackAlbum.ascii());			//ooo
			m_trackAlbum = QString::fromLocal8Bit(trackAlbum.toLatin1().data());		//xxx
			//m_trackComment = QString::fromLocal8Bit(trackComment.ascii());		//ooo
			m_trackComment = QString::fromLocal8Bit(trackComment.toLatin1().data());	//xxx
		}
		//if (CodecUtf8->heuristicContentMatch(trackArtist.ascii(), trackArtist.length()) >= 0)		//ooo
		//	m_trackArtist = QString::fromUtf8(trackArtist.ascii());					//ooo
		//if (CodecUtf8->heuristicContentMatch(trackAlbum.ascii(), trackAlbum.length()) >= 0)		//ooo
		//	m_trackAlbum = QString::fromUtf8(trackAlbum.ascii());					//ooo
		//if (CodecUtf8->heuristicContentMatch(trackComment.ascii(), trackComment.length()) >= 0)	//ooo
		//	m_trackComment = QString::fromUtf8(trackComment.ascii());				//ooo
...
QString KXineWidget::getXineLog() const
{
	QString logStr;
	int i = 0;
	//Q3TextStream ts(&logStr, QIODevice::WriteOnly);	//ooo
	QTextStream ts(&logStr, QIODevice::WriteOnly);		//xxx
...
void KXineWidget::showOSDMessage(const QString& message, uint duration, int priority)
{
	...
	//xine_osd_draw_text(m_osd, 0, 0, message.local8Bit(), XINE_OSD_TEXT1);		//ooo
	xine_osd_draw_text(m_osd, 0, 0, message.toLocal8Bit().data(), XINE_OSD_TEXT1);		//xxx
...
//void KXineWidget::slotEnableVideoFilters(bool enable)		//ooo
void KXineWidget::slotEnableVideoFilters(int enable)		//xxx
{
#ifndef USE_QT_ONLY
...
//void KXineWidget::slotEnableAudioFilters(bool enable)		//ooo
void KXineWidget::slotEnableAudioFilters(int enable)		//xxx
{
#ifndef USE_QT_ONLY
...
void KXineWidget::slotSetVisualPlugin(const QString& visual)
{
	...
	if ( (xine_get_status(m_xineStream ) == XINE_STATUS_PLAY)
			//&& (!xine_get_stream_info(m_xineStream, XINE_STREAM_INFO_HAS_VIDEO)) && (m_visualPluginName) )			//ooo
			&& ( ! xine_get_stream_info( m_xineStream, XINE_STREAM_INFO_HAS_VIDEO)) && ( ! m_visualPluginName.isEmpty ( ) ) )	//xxx
	{
		//m_xinePost = xine_post_init(m_xineEngine, m_visualPluginName, 0, &m_audioDriver, &m_videoDriver);			//ooo
		m_xinePost = xine_post_init(m_xineEngine, m_visualPluginName.toLatin1().data(), 0, &m_audioDriver, &m_videoDriver);	//xxx
		m_postAudioSource = xine_get_audio_source(m_xineStream);
		m_postInput = (xine_post_in_t*)xine_post_input(m_xinePost, const_cast<char*>("audio in"));
		xine_post_wire(m_postAudioSource, m_postInput);
	}
...
#else
	//urls = xine_get_autoplay_mrls(m_xineEngine, plugin.ascii(), &num);					//ooo
	urls = const_cast<char **>(xine_get_autoplay_mrls(m_xineEngine, plugin.toLatin1().data(), &num));	//xxx
#endif
...
void KXineWidget::mouseMoveEvent(QMouseEvent* mev)
{
	...
	//if (cursor().shape() == Qt::BlankCursor)	//ooo
	if (QCursor().shape() == Qt::BlankCursor)	//xxx
	{
		setCursor(QCursor(Qt::ArrowCursor));	//ooo
		//QCursor().setShape(Qt::ArrowCursor);	//xxx
	}
...
{
	if (!m_xineReady) return;
	//int cur = cursor().shape();	//ooo
	int cur = QCursor().shape();	//xxx
...
void KXineWidget::wheelEvent(QWheelEvent* e)
{
	...
	//float offset = log10( QABS(e->delta()) ) / 0.002;	//ooo
	float offset = log10( qAbs(e->delta()) ) / 0.002;	//xxx
	int newVal = 0;
	if (e->delta()>0)
...
void KXineWidget::slotSendPosition()
{
	...
	QTime posTime = msToTime(time);
	QString qsStyle ("hh:mm:ss.zzz");
	emit signalNewPosition((long)time);
	emit signalNewPosition(pos, posTime);
	emit signalNewPosition(pos, posTime.toString(qsStyle));
...
void KXineWidget::slotSeekToPosition(int pos)
{
	...
	emit signalNewPosition((long)length);
	emit signalNewPosition(pos, time);
	emit signalNewPosition(pos, time.toString(qsStyle));
...
void KXineWidget::slotStopSeeking()
{
	...
	//m_posTimer.start(500, false);		//ooo
	m_posTimer.start(500);			//xxx
}
...
//void KXineWidget::slotEnableAutoresize(bool enable)	//ooo
void KXineWidget::slotEnableAutoresize(int enable)	//xxx
{
...
const xine_t *KXineWidget::getXineEngine ( ) const
{
	return m_xineEngine;
}
...
//void KXineWidget::slotSetVolumeGain(bool gain)	//ooo
void KXineWidget::slotSetVolumeGain(int gain)		//xxx
{
	int amp = 0;
...
{
	//if (cursor().shape() == Qt::ArrowCursor)	//ooo
	if (QCursor().shape() == Qt::ArrowCursor)	//xxx
	{
		setCursor(QCursor(Qt::BlankCursor));	//ooo
		//QCursor().setShape(Qt::BlankCursor);	//xxx
	}
}
...
QImage KXineWidget::getScreenshot() const
{
	...
	//QImage screenShot(rgbPile, width, height, 32, 0, 0, QImage::IgnoreEndian);   //ooo
    QImage screenShot(rgbPile, width, height, QImage::Format_RGB32, 0, 0);         //xxx 
	if (scaleFactor >= 1.0)
	...
	//screenShot = screenShot.smoothScale(width, height);		//ooo
	screenShot = screenShot.scaled(width, height);			//xxx
...
void KXineWidget::warningOut (QString qsWarning)
{
	...
	//printf ((const char *)qsWarninging);		//ooo
	printf ("%s\n",qsWarninging.toLatin1().data());	//xxx
#else
